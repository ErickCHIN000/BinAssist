...
"""
LM Studio Provider - Implementation for LM Studio API
LM Studio provides an OpenAI-compatible API for local LLM models
"""
...
class LMStudioProvider(BaseLLMProvider):
    """
    LM Studio API provider implementation.
    
    LM Studio provides an OpenAI-compatible API for running local models.
    It typically runs on http://localhost:1234 and doesn't require API keys.
    Supports chat completions, streaming, and limited tool calling.
    """
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Set default URL if not provided (LM Studio's default)
        if not self.url or self.url in ['http://localhost:1234', 'http://localhost:1234/', '']:
        ...
        try:
            ...
            if self.disable_tls:
                import httpx
                import ssl
                log.log_warn(f"TLS verification disabled for LM Studio provider '{self.name}'")
                ...
                self._client = OpenAI(
                    api_key=self.api_key or "lm-studio",  # LM Studio doesn't require real API key
                    base_url=self.url,
                    timeout=30.0,
            ...
            raise APIProviderError(f"Failed to initialize LM Studio client: {e}")
        
        log.log_info(f"LM Studio provider initialized with URL: {self.url}, model: {self.model}")
    ...
    async def chat_completion(self, request: ChatRequest,
        ...
        log.log_info(f"LM Studio chat completion for {self.model} with {len(request.messages)} messages")
    ...
    async def _chat_completion_impl(self, request: ChatRequest,
        ...
        try:
            ...
            # Add tools if present (LM Studio has limited tool support)
            if request.tools:
            ...
            if not response.choices:
                raise APIProviderError("No choices in LM Studio response")
            ...
            # Call native message callback with actual LM Studio response
            if native_message_callback:
                # Convert the actual LM Studio response to dict for storage
            ...
            raise AuthenticationError(f"LM Studio authentication failed: {e}")
        except openai.RateLimitError as e:
            log.log_error(f"Rate limit error: {e}")
            raise RateLimitError(f"LM Studio rate limit exceeded: {e}")
            ...
            raise NetworkError(f"LM Studio connection failed: {e}")
        except openai.APIError as e:
            log.log_error(f"LM Studio API error: {e}")
            raise APIProviderError(f"LM Studio API error: {e}")
        except Exception as e:
    ...
    async def chat_completion_stream(self, request: ChatRequest,
        ...
        log.log_info(f"LM Studio streaming completion for {self.model} with {len(request.messages)} messages")
            ...
            try:
                ...
                for chunk in stream:
                    ...
                    if choice.finish_reason and not finished:
                        ...
                        if native_message_callback:
                            # Reconstruct LM Studio format from streaming data
                            native_message = {
                                "role": "assistant",
                ...
                log.log_error(f"Error iterating LM Studio stream: {stream_error}")
    ...
    async def generate_embeddings(self, request: EmbeddingRequest) -> EmbeddingResponse:
        """
        Generate embeddings using LM Studio's embedding API.
        
        Note: LM Studio's embedding support depends on the loaded model.
        Not all models support embeddings.
        """
        log.log_info(f"LM Studio embeddings for {len(request.texts)} texts")
        
        try:
            # LM Studio may or may not have an embedding model loaded
            # We'll use the main model or a default embedding model
            embedding_model = getattr(self, 'embedding_model', self.model)
            ...
            log.log_error(f"LM Studio API error getting embeddings: {e}")
            # LM Studio might not support embeddings with the current model
            raise NotImplementedError(f"LM Studio embeddings not supported: {e}")
        except Exception as e:
    ...
    def get_capabilities(self) -> ProviderCapabilities:
        ...
        return ProviderCapabilities(
            ...
            supports_embeddings=True,  # Model-dependent
            supports_vision=False,  # Model-dependent, not typically supported
    ...
    def validate_config(self):
        ...
        if not self.model:
            raise ValueError("Model is required for LM Studio provider")
        
        # API key is not required for LM Studio (typically runs locally)
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            ...
            log.log_error(f"LM Studio connection test failed: {e}")
            return False


# Factory for LM Studio provider
from ..llm_providers.provider_factory import ProviderFactory
from ..models.provider_types import ProviderType

class LMStudioProviderFactory(ProviderFactory):
    """Factory for creating LM Studio provider instances"""
    
    def create_provider(self, config: Dict[str, Any]) -> LMStudioProvider:
    ...
    def supports_provider_type(self, provider_type: ProviderType) -> bool:
        ...
        return provider_type == ProviderType.LMSTUDIO
